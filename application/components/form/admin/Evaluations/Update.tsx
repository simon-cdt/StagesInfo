...
import { updateRateAdmin } from "@/lib/actions/admin/evaluation";
...
export default function UpdateRateDialogAdminForm({
  id,
  defaultValue,
  refetch,
}: {
  id: string;
  defaultValue: { id: string; rating: number; comment: string };
  refetch: () => void;
}) {
  ...
  const onSubmit = async (data: FormSchema) => {
    const response = await updateRateAdmin({
      id,
      rating: data.rating,
      comment: data.comment,
    });
    if (response.success) {
      toast.success("L'évaluation a été mise à jour avec succès.");
      refetch();
      setIsOpen(false);
    } else {
