...
import { updateOfferAdmin } from "@/lib/actions/admin/offer";
...
export default function UpdateOfferAdminForm({
  id,
  title,
  description,
  sectorId,
  ...
  const { isError, data: sector, isLoading } = useSectors();
  ...
  const [skills, setSkills] = useState<string[]>(initialSkills.split(","));
  ...
  const handleSubmitForm = async (data: FormSchema) => {
    const response = await updateOfferAdmin({
      id,
      title: data.title,
      ...
      <DialogTrigger asChild>
        ...
        >
          <div className="flex items-center gap-1">
            <Icon src="page-edit" />
            <p>{disabled ? "Interdit" : "Modifier le stage"}</p>
          </div>
        </Button>
      </DialogTrigger>
